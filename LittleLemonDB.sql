...
CREATE TABLE `bookings` (
  ...
  KEY `CustomerID_fk_idx` (`CustumerID`),
  CONSTRAINT `CustomerID_fk` FOREIGN KEY (`CustumerID`) REFERENCES `customers` (`CustomerID`) ON DELETE CASCADE ON UPDATE CASCADE
...
CREATE TABLE `menus` (
  ...
  PRIMARY KEY (`MenuID`),
  KEY `MenuItemsID_fk_idx` (`MenuItemsID`),
  CONSTRAINT `MenuItemsID_fk` FOREIGN KEY (`MenuItemsID`) REFERENCES `menuitems` (`MenuItemsID`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3;
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `orders` (
  ...
  CONSTRAINT `CustomerIDorder_fk` FOREIGN KEY (`CustomerID`) REFERENCES `customers` (`CustomerID`) ON DELETE CASCADE ON UPDATE CASCADE,
...
-- Dump completed on 2023-04-27  5:45:40
